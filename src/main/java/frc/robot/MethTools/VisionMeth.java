...
public class VisionMeth {
    ...
    {
        return PhotonUtils.calculateDistanceToTargetMeters(0.7, Physics.hub_height,Math.toRadians(20), Math.toRadians(camera.getLatestResult().getBestTarget().getYaw())) - (Physics.hub_diameter / 3);
    }
    public static double quarticDistance(PhotonCamera camera) throws NullPointerException
    {
        if (camera.getLatestResult().hasTargets())
        {
            ...
            double real_height = 2.3; double camera_height = 0.72;
